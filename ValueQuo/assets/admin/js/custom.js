...
changeAStatus = function (k, i, t) {
    chStatus = {key: k, id: i, tab: t};
    $.ajax({
        url: site_url + "admin/Admin_controller/setAStatus",
        data: $.param(chStatus),
        type: 'POST',
        dataType: 'json',
        success: function (data) {
...
exceldownload = function () {
    var queryString = $('#xform').serialize();
    if ($('.multi-select').is(":checked")) {
        window.location = site_url + "admin/Admin_controller/exceldownload?" + queryString;
    } else {
        alert("Please Check at least one checkbox!!");
    }
    return false;
...
function ticketadmin(x) {
    if (x == "" || x == null) {
        return "<a href='javascript:;' class='btn btn-success btn-sm'>Open</a>";
    } else {
        return x;
    }
}

assignTicketToAdmin = function (field, id, table) {
    queryString = {field: field, id: id, table: table};
    if (confirm("Are you sure to open a ticket?)")) {
        $.ajax({
            url: site_url + "admin/Ticket_controller/assignTicketToAdmin",
...
function homeimage(x) {
    return "<img style='width:728px;height:90px;'></img>";
}
